...
(function (window) {
  ...
  const Social = {
    ...
    update: (id, patch) => api(`social/${id}`, { method: "PATCH", body: patch }),
    remove: (id) => api(`social/${id}`, { method: "DELETE" }),
    toggle: async (id) => {
  ...
  const Products = {
    ...
    update: (id, patch) => api(`products/${id}`, { method: "PATCH", body: patch }),
    ...
    reorder: (orderedIds) => api("products/reorder", { method: "POST", body: { ids: orderedIds } }),
    ...
    recordClick: (id) => api("products/click", { method: "POST", body: { id } }),
  };
  ...
  // ---------------- Analytics ----------------
  const Analytics = {
    logVisit: () => api("analytics/visit", { method: "POST" }).catch(() => {}),
  ...
  const Auth = {
    ...
    isLoggedIn: async () => {
      const { loggedIn } = await api("auth/session");
      return loggedIn;
    },
    requireLogin: async () => {
